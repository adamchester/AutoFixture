...
namespace Ploeh.AutoFixture.Kernel
{
    ...
    public class EnumerableRelay : ISpecimenBuilder
    {
        ...
        public object Create(object request, ISpecimenContext context)
        {
            ...
            if (typeArgs.Length != 1) return NoSpecimen.Instance;
            if (type.GetGenericTypeDefinition() != typeof(IEnumerable<>)) return NoSpecimen.Instance;
            var enumerable = context.Resolve(new MultipleRequest(typeArgs[0])) as IEnumerable<object>;
            if (enumerable == null) return NoSpecimen.Instance;
